...
class ClutchNodeBuilder {
  ...
  protected function addQuickeditAttributeForBundle($crawler, $component) {
    ...
    $quickedit = 'node/'. $component->id();
    $bundle_layer = $crawler->filter('[data-content-type="'. $bundle .'"]');
    $bundle_layer->setAttribute('data-quickedit-entity-id', $quickedit)->addClass('contextual-region');

    $build_contextual_links['#contextual_links']['node'] = array(
      'route_parameters' =>array('node' => $component->id()),
  ...
  public function deleteEntities($bundles) {
    foreach($bundles as $bundle) {
      ...
      if($entity_id) {
        ...
        entity_load('nodes', $entity_id)->delete();
  ...
  public function getDescription(Crawler $crawler) {
    $bundle_descrip = $crawler->filter('*')->getAttribute('data-description');
    return  $bundle_descrip;
  }
  /**
